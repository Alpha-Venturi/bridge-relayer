...
const wallet = new ethers_1.ethers.Wallet(OPERATOR_PRIVATE_KEY);
const network1Provider = new ethers_1.ethers.providers.JsonRpcProvider(NETWORK_1_JSON_RPC_URL);
const network1Signer = wallet.connect(network1Provider);
const network2Provider = new ethers_1.ethers.providers.JsonRpcProvider(NETWORK_2_JSON_RPC_URL);
const network2Signer = wallet.connect(network2Provider);
const bridge1 = typechain_types_1.BridgeTimelockController__factory.connect(NETWORK_1_BRIDGE_ADDRESS, network1Signer);
const bridge2 = typechain_types_1.BridgeTimelockController__factory.connect(NETWORK_2_BRIDGE_ADDRESS, network2Signer);
...
bridge1.on(sourceTransferInitFilter1, async (recipient, targetToken, targetChainId, value, commitment, sender, sourceToken) => {
    try {
        const token = await typechain_types_1.ERC20__factory.connect(targetToken, network2Signer);
...
bridge2.on(sourceTransferInitFilter2, async (recipient, targetToken, targetChainId, value, commitment, sender, sourceToken) => {
    try {
        const token = await typechain_types_1.ERC20__factory.connect(targetToken, network1Signer);
...
const run = async () => {
    while (true) {
        ...
        await (0, promises_1.setTimeout)(1000000);
    }
};
